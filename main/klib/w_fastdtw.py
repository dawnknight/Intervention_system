# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 23:05:29 2017
...
def fastdtw(x, y,w, radius=1, dist=None):
    ...
    return __fastdtw(x, y,w, radius, dist)
...
def __fastdtw(x, y,w, radius, dist):
    min_time_size = radius + 2

    if len(x) < min_time_size or len(y) < min_time_size:
        return dtw(x, y,w, dist=dist)

    x_shrinked = __reduce_by_half(x)
    y_shrinked = __reduce_by_half(y)
    distance, path = \
        __fastdtw(x_shrinked,y_shrinked,w, radius=radius, dist=dist)
    window = __expand_window(path, len(x), len(y), radius)
    return dtw(x, y,w, window, dist=dist)


def dtw(x, y,w, window=None, dist=lambda a, b: abs(a - b)):
